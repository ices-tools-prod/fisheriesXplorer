...
#   w <- tryCatch({
#     if (!is.null(session)) {
#       session$clientData[["output_landings_1-landings_plot_width"]]
#     } else {
#       NA_real_
#     }
#   }, error = function(e) NA_real_)
...
#   # Filter only when plotting main landed species
#   if (type == "Common name") {
...
#   # Total line based on the same filtered dataset used in the plot
#   total_df <- df %>%
#     dplyr::group_by(Year) %>%
...
#   file_stub <- paste0(
#     sanitize_stub(ecoregion %||% "ecoregion"),
#     "_landings_",
#     sanitize_stub(type),
#     if (!is.null(selected_guild) && nzchar(selected_guild)) {
...
#         automargin = TRUE
#       ),
#       margin = list(l = 80, r = 20, t = 70, b = 100),
#       annotations = list(
#         list(
